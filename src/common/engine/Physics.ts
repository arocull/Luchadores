...
export function TickPhysics(DeltaTime: number, fighters: Fighter[], map: Map) {
  for (var i = 0; i < fighters.length; i++) {
    ...
    let deltaX = Vector.Add(Vector.Multiply(accel, Math.pow(DeltaTime, 2) / 2), Vector.Multiply(obj.Velocity, DeltaTime));

    // If they attempted to move faster than their max momentum, clamp their movement (should do this?)
    if (deltaX.length() > obj.MaxMomentum / obj.Mass) deltaX = Vector.Multiply(Vector.UnitVector(deltaX), (obj.MaxMomentum / obj.Mass));
  ...
  for (var i = 0; i < fighters.length; i++) {
    for (let j = i + 1; j < fighters.length; j++) { // If the entity was already iterated through by main loop, should not need to do it again
      const a = fighters[i];
      const b = fighters[j];
      if (Vector.Distance(a.Position, b.Position) <= a.Radius + b.Radius && (a.Position.z <= b.Position.z + b.Height)) { // If they are within collision range...
